...
public class UserDetailsImpl implements UserDetails {
    ...
    public static UserDetailsImpl build(Utilisateur user, Set<Roles> roles) {
        List<GrantedAuthority> authorities = roles.stream()
                .map(role -> new SimpleGrantedAuthority(role.getNom()))
                .collect(Collectors.toList());
        return new UserDetailsImpl(
                user.getId(),
                user.getEmail() == null ? user.getNom() : user.getEmail(),
                user.getEmail() == null ? user.getNumero() : user.getEmail(),
                user.getPassWord(),
                authorities);
    }
