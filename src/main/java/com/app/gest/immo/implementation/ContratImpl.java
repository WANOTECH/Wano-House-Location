...
public class ContratImpl implements IContrat, ILoyer{
	...
	public List<ContratDTO> findByDateMiseEnLoyer(Date dateMiseEnPlace) throws Exception {
		List<Contrat> listContrat = iContrat.findContratByDateMiseEnplace(dateMiseEnPlace);
		List<ContratDTO> listContratDTO = new ArrayList<>();
		if(listContrat==null || listContrat.isEmpty()){
			throw  new Exception("");
		}
		for (Contrat contrat : listContrat){
			listContratDTO.add(toDTO(contrat));
	...
	public LoyerDTO findLoyerByCode(String code) throws Exception {
		List<Loyer> listLoyer = iLoyer.findLoyerByCode(code);
		if(listLoyer==null || listLoyer.isEmpty()){
			return null;
		}
		Loyer loyer = listLoyer.get(0);
		return setLoyerDTO(loyer);
	}
	...
	public List<LoyerDTO> findLoyerByContrat(Contrat contrat) throws Exception {
		List<Loyer> listLoyer = iLoyer.findByContrat(contrat);
		if (listLoyer==null || listLoyer.isEmpty()){
